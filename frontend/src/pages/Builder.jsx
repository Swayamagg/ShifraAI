...
const saveAssistant=async () => {
  try {
    const data={assistantName,businessName,businessType,tone,theme,geminiApiKey,businessDesc,pages}
...
const embedCode=`<script src="${CLIENT_URL}/assistant.js" data-user-id="${user._id}"></script>`
  return (
    <div className='min-h-screen bg-[#f7f8fc] px-4 py-8'>
      <div className='max-w-4xl mx-auto'>
        ...
        {user.isSetupComplete && !editAssistant && (<div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6 mb-6'>
           <p className='text-gray-400 text-sm'>Assistant</p>
           <h2 className='text-3xl font-bold text-[#081028] mt-1'>{user.assistantName}</h2>
           <p className='mt-3 leading-7 text-gray-500'>Your assistant is ready to use on your website</p>
           ...
           <div className='mt-7'>
            <div className='mt-4 rounded-2xl bg-amber-50 border border-amber-200 p-4'>
               ...
               <p className='text-sm text-amber-700 leading-6 mt-2'>
               ...
               <pre className='mt-3 bg-[#0b1020] rounded-xl p-3 text-emerald-400 text-xs font-mono overflow-x-auto'>
                {`<body>
 ...
 <script src="${CLIENT_URL}/assistant.js" data-user-id="${user?._id}"></script>
            ...
            <p className='text-sm font-medium mb-3 mt-3 text-[#081028]'>Embed Code</p>
           </div>
           <div className='relative'>
            <textarea className='w-full h-20 bg-[#0b1020] text-emerald-400 rounded-2xl p-4 text-sm font-mono resize-none outline-none' value={embedCode} ></textarea>
            <button onClick={()=>{
        ...
        {editAssistant && <div className='space-y-6'>
          ...
          <div className='bg-white rounded-3xl border border-gray-100 shadow-sm p-6'>
              <div className='flex items-center justify-between mb-5 flex-wrap'>
                ...
                <button onClick={addPage} className='flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-emerald-500 text-white text-sm'>
                  <FiPlus/>Add
                </button>
              </div>
              <div className='grid grid-cols-1 sm:grid-cols-3 gap-3'>
                <input onChange={(e)=>setPageName(e.target.value)} value={pageName} type="text" placeholder='Page Name' className='border border-gray-200 rounded-2xl px-4 py-3' />
                <input onChange={(e)=>setPagePath(e.target.value)} value={pagePath} type="text" placeholder='/pricing' className='border border-gray-200 rounded-2xl px-4 py-3' />
                <input onChange={(e)=>setPageKeywords(e.target.value)} value={pageKeywords} type="text" placeholder='Pricing , Plan' className='border border-gray-200 rounded-2xl px-4 py-3' />
              </div>
